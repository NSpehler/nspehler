 ...
 *
 * @see app/api/invalidate-cache/route.ts for webhook-based cache invalidation
 */
export async function executeQuery<Result, Variables>(
  ...
  const result = await libExecuteQuery(query, {
    ...
    baseEditingUrl:
      process.env.DATOCMS_BASE_EDITING_URL || "https://admin.nspehler.com",
    requestInitOptions: {
      cache: "force-cache",
      /*
       * This project uses an extremely basic cache invalidation technique: by
       * using the `next.tags` option, all requests to DatoCMS are tagged with
       * "datocms" in the Next.js Data Cache. Whenever DatoCMS notifies us of
       * any updates via webhook, we invalidate all requests with the same tag.
       *
